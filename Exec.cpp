...
namespace csci331_project1
{
	...
	void Exec::run_Kway()
	{
		...
		ifstream in_file;
		ofstream out_file;
		in_file.open(RunFile.c_str());
		out_file.open(OutputFile.c_str());
		cout << "/nEnter which type of Keys";
		cout << "0: Integers  1: Strings  2: Pairs";
		cin >> KwaySort.Key_type;
		cout << "\nRunning K Way Tournament Merge...\n";
		KwaySort.Get_Inputs(in_file);
		KwaySort.sort_and_create(out_file);
